...
class TestModel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_simple(self):
        response = await self.model.get_response(self.data['simple'], [])
    ...
    async def test_reply(self):
        response = await self.model.get_response(self.data['reply'], [("ИДЗ 4", datetime.strptime("21.05.2025 23:00:00", "%d.%m.%Y %H:%M:%S"))])
    ...
    async def test_complex_date(self):
        response = await self.model.get_response(self.data['complex_date'], [])
